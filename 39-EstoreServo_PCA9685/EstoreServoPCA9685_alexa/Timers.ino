...
void control_rele(int j, int Output){
  ...
  Output=Output; //en el NodemCu van al revés... (modificado a logiaga positiva)
    ...
    //si ya se ha actuado, entonces no hacer nada hasta el próximo cambio
    //permite otros controles (alexa, mqtt)
    if(timers_estado_anterior==Output){return;}
    timers_estado_anterior=Output;
    Serial.print("Actuando Salida a:"); Serial.println(Output);
    //digitalWrite(GPIOPIN[j],Output);
   switch( j){
    case 0:
      //digitalWrite(Relay_D5,Output);
      //Serial.print("Activado rele0 : ");Serial.println(Output);
      controlestore(Output);
      break;
    case 1:
     //digitalWrite(Relay_D6,Output);
     //Serial.print("Activado rele1 : ");Serial.println(Output);
     break;
//    case 2:
