 ...
 * 
 * Sensor de irradiancia 2020
 * 
 * 
 * ADS1115 para la lectura del voltaje de salida del sensor G
...
#include "variables.h" 
#include "G.h"  //para el sistema 1, TDS DS18B20 sensor de temperatura con control calentador y ventilador
...
void setup() {
  ...
  conf.EEPROM_chk++; //contabiliza el número de arranques
...
void loop() {
  ...
  server_read();
  horasolar();
  //controlG(); se hace desde los timers cada minuto!
  controlRelleno();
  sendthingspeak();
  sendtoemoncms();
