  ...
  * void GravityTDS::update()
{
  ...
  this->decValue=(133.42*this->voltage*this->voltage*this->voltage - 255.86*this->voltage*this->voltage + 857.39*this->voltage)*this->kValue;
...
//  float multiplier = 0.125F;  // ganancia 1
 //float multiplier = 0.03125F; //ganancia 4
...
void activarRelTemp(int e1, int e2){
  estados[4]=e1;
  estados[5]=e2;
  //DPRINTLN("estados e1");DPRINTLN(e1);
  //DPRINTLN("estados e2");DPRINTLN(e2);
...
void controltemperatura(){
  ...
  if (temperatureC > conf.temp_max+0.5){  //medio grado de histéresis
    activarRelTemp(0, 1);
    }
    if (temperatureC < conf.temp_min-0.5){
     activarRelTemp(1, 0);
    }
